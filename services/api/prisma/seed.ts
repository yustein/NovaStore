...
async function main(): Promise<void> {
  ...
  await prisma.user.upsert({
    where: { email: "admin@novastore.local" },
    update: { displayName: "Dev Admin", status: "active" },
    create: {
      id: "usr_dev_admin",
      email: "admin@novastore.local",
  ...
  for (const seed of releaseSeeds) {
    await prisma.app.upsert({
      where: { packageName: seed.packageName },
      update: {
        name: seed.name,
    ...
    await prisma.artifact.upsert({
      ...
      update: {
        appId: seed.id,
        uploadedByUserId: "usr_dev_admin",
        artifactType: "apk",
